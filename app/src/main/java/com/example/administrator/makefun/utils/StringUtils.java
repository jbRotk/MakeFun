...
public class StringUtils {
	...
	public static boolean isIDCard(String idcard) {
		String cardRegex = "^(\\d{15}$|^\\d{18}$|^\\d{17}(\\d|X|x))$";
		if (isEmpty2(idcard))
			return false;
		else
			return idcard.matches(cardRegex);
	}
	...
	public static float setBigDecimal(int scale, float f) {
		BigDecimal bigDecimal = new BigDecimal(f);
		float floatValue = bigDecimal.setScale(scale, BigDecimal.ROUND_HALF_UP)
				.floatValue();
		return floatValue;
	}
